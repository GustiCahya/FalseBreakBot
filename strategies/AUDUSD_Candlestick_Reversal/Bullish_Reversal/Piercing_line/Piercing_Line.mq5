...
//--- Plot Arrow
#property indicator_label1  "Piercing Line"
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Piercing Line");
                ...
                const int &spread[])
{
   ...
   for(int i = start; i < rates_total; i++)
   {
      ...
      double prevBody = open[i-1] - close[i-1];  // positive for bearish
      if(prevBody <= 0) continue;
      
      //--- Current open below previous low (or close)
      bool gapDown = (open[i] <= close[i-1]);
      ...
      if(gapDown && pierces && IsDowntrend(i - 1, close, open))
      {
