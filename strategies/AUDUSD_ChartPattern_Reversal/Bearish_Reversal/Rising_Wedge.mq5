...
//|                        Rising Wedge Pattern Indicator             |
//+------------------------------------------------------------------+
...
//--- Plot Arrow
#property indicator_label1  "Rising Wedge"
...
input double InpConvergeTol    = 0.0001; // Min convergence rate per bar
...
int OnInit()
{
   ...
   IndicatorSetString(INDICATOR_SHORTNAME, "Rising Wedge");
...
//+------------------------------------------------------------------+
//| Linear regression slope and intercept over swing points           |
//+------------------------------------------------------------------+
                ...
                const int &spread[])
{
   ...
   for(int i = start; i < rates_total - InpSwingLookback; i++)
   {
      ...
      if(slopeH <= 0 || slopeL <= 0)           continue;  // must both slope up
      if(slopeH >= slopeL)                      continue;  // upper TL must be flatter → converging

      //--- Lower trendline at bar i
      double lowerTLAtI = slopeL * i + intL;

      //--- Confirmation: close breaks below lower trendline
      if(close[i] < lowerTLAtI)
      {
